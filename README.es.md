<p align="center">
  🇺🇸 <a href="./README.md">English</a> |
  🇲🇽 <a href="./README.es.md">Español</a>
</p>

<h1 align="center">Hola 👋, soy Yavé Emmanuel Vargas Márquez</h1>

<h3 align="center">
Científico de Datos | Entusiasta de IA y NLP
</h3>

<p align="center">
Ingeniero en Sistemas Computacionales enfocado en Ciencia de Datos, Machine Learning, NLP y sistemas backend impulsados por IA.
</p>

<p align="center">
  <a href="https://yave-vargas-portfolio.vercel.app" target="_blank">
    🌐 Portafolio
  </a>
  •
  <a href="https://www.linkedin.com/in/yave-emmanuel-vargas-marquez/" target="_blank">
    LinkedIn
  </a>
  •
  <a href="mailto:yave.vargas16@gmail.com">
    Email
  </a>
  •
  <a href="https://github.com/Yave-Vargas" target="_blank">
    GitHub
  </a>
</p>

---

# 🧠 Sobre mí

🎓 Ingeniero en Sistemas Computacionales por el IPN – UPIIZ  
📍 Zacatecas, México  

Desarrollo soluciones de Ciencia de Datos y NLP utilizando Machine Learning y Modelos Grandes de Lenguaje.

Mi experiencia incluye:
- Sistemas backend impulsados por IA
- APIs REST con FastAPI
- Pipelines NLP para procesamiento de PDFs
- Inferencia local con LLMs/SLMs usando Ollama
- Análisis y visualización de datos
- Evaluación y optimización de modelos

Me interesa especialmente:
- Ciencia de Datos
- NLP y LLMs
- Machine Learning
- Sistemas backend
- Automatización con IA

---

# 🔬 Proyecto Destacado

## YEGI — Plataforma de Resumización con IA

Plataforma web y API para resumización de artículos científicos usando LLMs y SLMs.

### Características
- Extracción de texto desde PDFs científicos
- Pipelines de preprocesamiento NLP
- Inferencia local con Ollama
- Arquitectura REST API con FastAPI
- Optimización de hiperparámetros para mejorar la calidad de los resúmenes

### Tecnologías
Python · FastAPI · Ollama · PyTorch · NLP · Docker · PostgreSQL

### Investigación y Publicaciones
📄 Publicación en IEEE Xplore:  
https://ieeexplore.ieee.org/document/11311884

📘 Capítulo de libro académico:  
http://ricaxcan.uaz.edu.mx/jspui/handle/20.500.11845/4203

---

# 🛠 Habilidades Técnicas

<table align="center">
<tr>

<td valign="top" align="center" width="260">

### Lenguajes

<p align="center">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg" width="42"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/javascript/javascript-original.svg" width="42"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/java/java-original.svg" width="42"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/c/c-original.svg" width="42"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/cplusplus/cplusplus-original.svg" width="42"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/bash/bash-original.svg" width="42"/>
</p>

Python · JavaScript · Java · C · C++ · Bash

</td>

<td valign="top" align="center" width="260">

### Ciencia de Datos e IA

<p align="center">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/pandas/pandas-original.svg" width="42"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/numpy/numpy-original.svg" width="42"/>
  <img src="https://upload.wikimedia.org/wikipedia/commons/0/05/Scikit_learn_logo_small.svg" width="42"/>
  <img src="https://www.vectorlogo.zone/logos/pytorch/pytorch-icon.svg" width="42"/>
  <img src="https://www.vectorlogo.zone/logos/opencv/opencv-icon.svg" width="42"/>
</p>

Pandas · NumPy · Scikit-learn · PyTorch · OpenCV

</td>

<td valign="top" align="center" width="260">

### Backend y Bases de Datos

<p align="center">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/fastapi/fastapi-original.svg" width="42"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/postgresql/postgresql-original-wordmark.svg" width="42"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original-wordmark.svg" width="42"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/docker/docker-original-wordmark.svg" width="42"/>
</p>

FastAPI · PostgreSQL · MySQL · Docker

</td>

</tr>

<tr>

<td colspan="3" valign="top" align="center">

### Visualización y Analítica

<p align="center">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/pandas/pandas-original.svg" width="42"/>
  <img src="https://seaborn.pydata.org/_images/logo-mark-lightbg.svg" width="42"/>
  <img src="https://images.plot.ly/logo/new-branding/plotly-logomark.png" width="42"/>
  <img src="https://upload.wikimedia.org/wikipedia/commons/c/cf/New_Power_BI_Logo.svg" width="42"/>
</p>

Pandas · Seaborn · Plotly · Power BI

</td>

</tr>
</table>

---

# 📈 Actualmente Aprendiendo

- Evaluación de LLMs
- Pipelines NLP
- Automatización con IA
- Optimización de modelos de Machine Learning
- Sistemas de IA listos para producción

---

# 🎓 Educación

### Instituto Politécnico Nacional – UPIIZ
Ingeniería en Sistemas Computacionales  
2021 – 2025

### Instituto Politécnico Nacional – CECyT 18
Técnico en Sistemas Digitales  
2018 – 2021

---

# 🌎 Idiomas

- Español — Nativo
- Inglés — B2

---

# 📫 Contacto

📧 yave.vargas16@gmail.com

💼 LinkedIn  
https://www.linkedin.com/in/yave-emmanuel-vargas-marquez/

🌐 Portafolio  
https://yave-vargas-portfolio.vercel.app

---

⭐ Abierto a oportunidades en Ciencia de Datos, Análisis de datos, IA, NLP y desarrollo backend.
